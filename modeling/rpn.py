...
class RPN_CLIP(RPN):
    ...
    def forward(
        ...
        """
        deal_features = self.in_features.copy()
        if self.training:
            deal_features.append('res4_aug')
        features = [features[f] for f in deal_features]
        ...
        if self.training:
            ...
            losses = self.losses(
                anchors, pred_objectness_logits[0:1], gt_labels, pred_anchor_deltas[0:1], gt_boxes
            )
            losses.update({key+'_aug':value for key,value in self.losses(anchors, pred_objectness_logits[1:2], gt_labels, pred_anchor_deltas[1:2], gt_boxes).items()})
        else:
            losses = {}
        proposals = self.predict_proposals(
            anchors, pred_objectness_logits[0:1], pred_anchor_deltas[0:1], images.image_sizes
        )
        if self.training:
            proposals_aug = self.predict_proposals(
                anchors, pred_objectness_logits[1:2], pred_anchor_deltas[1:2], images.image_sizes
            )
            return proposals, losses, proposals_aug
        else:
